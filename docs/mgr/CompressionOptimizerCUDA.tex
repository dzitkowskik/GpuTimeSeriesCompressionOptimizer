...
\subsection{TS}
Jest to biblioteka definiuj¹ca strukturê szeregu czasowego, oraz udostêpniaj¹ca metody do odczytu i zapisu szeregów z plików binarnych oraz tekstowych o kolumnach rozdzielonych separatorem, na przyk³ad CSV. Ponadto umo¿liwia definicjê szeregu czasowego poprzez plik nag³ówkowy o strukturze wierszy: ([nazwa kolumny],[typ kolumny],[precyzja]), w której ka¿dy wiersz definiuje osobn¹ kolumnê. Przyk³adowa treœæ pliku nag³ówkowego zosta³a umieszczona na listingu~\ref{lst:tsheader}.
...
Aby kodowania, a raczej ich wynik by³ mo¿liwy do zserializowania, by potem odtworzyæ schemat u¿yty do kompresji, przed metadanymi wyniku dodawany jest header w postaci: \emph{[Typ Kodowania (16 bit), Typ danych (16 bit),
...
\newline
Nie bêdê tutaj opisywa³ algorytmu konwersji drzewa z i do takiego opisu, gdy¿ jest to
prosty algorytm rekurencyjny przechodzenia drzewa wg³¹b. Jednak sam schemat jest o tyle wa¿ny,
¿e w ten sposób bêd¹ u³o¿one fragmenty skompresowanych danych przez algorytm kompresji drzewem.
...
oraz nag³ówków. Ostatni¹ czynnoœci¹ w algorytmie jest po³¹czenie wych danych w jeden ci¹g wynikowy
...
Kolejny algorytm rekurencyjny, który na wejœciu dostaje SCBP ze skomprsowanymi danymi, oraz
offset - referencja do paramatru okreœlaj¹cego po³o¿enie ju¿ odczytanego fragmentu danych, tj.
...
do kompresji. Sama przebiega jak pokazuje algorytm \ref{alg:rec_tree}.
...
Aby podejmowaæ dobre decyzje optymalizator musi znaæ charakterystykê danych na których operuje.
G³ówne zastosowanie statystyk w tym systemie, opiera siê na przesiewaniu wszystkich mo¿liwych drzew,
do tych najistotniejszych, które mog¹ uzyskaæ dobry wspó³czynnik kompresji. Ponadto niektóre statystyki
wykorzystywane s¹ przez same kodowania, aby odpowiednio dzia³aæ. W tym systemie zaimplementowano statystyki
takie jak:
\begin{enumerate}
...
Jest to œrednia d³ugoœæ maksymalnych ci¹gów równych liczb, nie d³u¿szych ni¿ N w danych D, ozn. $RLE_{M}(D, N)$. Jest to ciekawa statystyka, mog¹ca wskazywaæ na zasadnoœæ u¿ycia kodowania RLE do jakiœ danych. Mo¿na uznaæ ¿e jeœli $RLE_{M}(D, N) > N/2$ to warto u¿yæ RLE do kodowania danych $D$. Dla przyk³adu jeœli weŸmiemy $N=2$ oraz dane D: $1112234444555555$ wtedy odczytane ci¹gi równych liczb nie d³u¿sze niæ 2 wynosz¹: $22121122212222$, z czego œrednia jest równa $\frac{26}{15} \approx 1,7$, natomiast bior¹c $N=4$: $3212114321444$ oraz œrednia $\frac{32}{13} \approx 2,5$. W obu wypadkach wychodzi zatem, ¿e powinniœmy u¿yæ RLE do kompresji takich danych. Daje to wyobra¿enie o œredniej d³ugoœci ci¹gów równych liczb w danych, a ponadto jest bardzo wydajne obliczeniowo. Wyliczanie takiej statystyki jest trywialnie równoleg³e, poniewa¿
mo¿na w ka¿dym w¹tku wyliczaæ ci¹g pocz¹wszy od innego indeksu, niezale¿nie. Pseudokod wyliczania ci¹gu dla pojedynczego w¹tku CUDA pokazuje algorytm \ref{alg:rle_metric}. PóŸniej wystarczy obliczyæ sumê, korzystaj¹c na przyk³ad z gotowej funkcji z biblioteki \emph{Thrust}.
...
\subsection{Precyzja}
Kolejn¹ wa¿n¹ statystyk¹ dla liczb zmiennoprzecinkowych jest precyzja, gdy¿ mówi czy warto zastosowaæ zmianê wartoœci na typ ca³kowitoliczbowy, stosuj¹c przemno¿enie przez odpowiedni¹ potêgê liczby 10 (patrz kodowanie \emph{FLOAT\_TO\_INT}). Analogicznie do metryki RLE algorytm wyliczania precyzji jest trywialnie równoleg³y, a obliczanie precyzji pokazuje algorytm \ref{alg:precision}.
...
Rekurencyjny algorytm generowania interesuj¹cych drzew na postawie statystyk. Wyjaœniê najpierw co to znaczy, ¿e drzewo jest interesuj¹ce. Jest to takie drzewo, którego ka¿dy wêze³ zosta³ wybrany jako dobra kontynuacja
wêz³a poprzedniego, wedle okreœlonych regu³. Regu³y tworzone s¹ na podstawie statystyk danych, które dany wêze³ ma dostaæ do kompresji, typu poprzedzaj¹cej go kompresji oraz typu danych. Dany jest tak¿e aktualny poziom drzewa na którym ma siê znaleŸæ nowy wêze³. Metodê zwracaj¹c¹ dobre kontynuacje nazwijmy roboczo \emph{GetContinuations} i zwraca ona listê typów kodowañ, które spe³niaj¹ regu³y. Kilka przyk³adów:
\begin{itemize}
\item Jeœli poprzednik kodowaniem nie by³o \emph{FLOAT\_TO\_INT}, a dane maj¹ nisk¹ precyzjê, ponadto typ danych to double i ich wartoœæ maksymalna jest niewielka, to dodaj do listy kontynuacji typ kodowania \emph{FLOAT\_TO\_INT}, poniewa¿ jest zasadne.
\item Jeœli poprzednim kodowaniem by³o \emph{GFC} albo \emph{AFL} to nie kompresuj ju¿ wiêcej i jako jedyn¹
mo¿liw¹ kontynuacjê zwróæ NONE. Jeœli nie, a typ danych to float lub double to na pewno dodaj \emph{GFC} do listy mo¿liwych kontynuacji, w p.p. dodaj \emph{AFL}. 
\end{itemize}
...
\normalsize
\noindent
Metody \emph{CrossTrees} tworz¹ wszystkie mo¿liwe kompinacje, w których $T$ jest wierzcho³kiem, a drzewa
z kolejnych dwóch argumentów (wektory drzew) ich poddrzewami. Dodatkowo uaktualniany jest wspó³czynnik 
kompresji po³¹czonych drzew. Przyk³adem dzia³ania tej metody jest rysunek \ref{ref:phase1}.
...
by³o interesuj¹ce, a nastêpnie kompresujemy danym kodowaniem dane i dla nich rekurencyjnie wyznaczamy interesuj¹ce drzewa. Nastêpnie ³¹czymy wyniki we wszystkie mo¿liwe kombinacje ale zachowuj¹c hierarchiê i 
do³¹czamy do mo¿liwych rozwi¹zañ. Przy okazji wiemy jak dobrze ka¿de skonstruowane drzewo zachowuje siê
dla danych wejœciowych. Na koñcu drzewo o najlepszej statystyce wybierane jest jako optymalne.

\subsection{Faza 2 - kompresja}
W tej fazie w³aœciwe dane (du¿a paczka danych) jest kompresowana przez drzewo wybrane jako optymalne, ale
co wa¿niejsze uaktualniane s¹ statystyki krawêdzi drzewa binarnego. W tej czêœci opiszê jak wygl¹daj¹ te
statystyki i jak s¹ uaktualniane. Wa¿ne jest równie¿, ¿e drzewo optymalne pamiêta ostatni¹ kopiê statystyk,
która jest tworzona w momencie ustawienia nowego drzewa jako optymalne.
    ...
    \end{figure}
\noindent
Krawêdzie w drzewie kompresji odpowiadaj¹ parom kompresji na pewnym miejscu w odpowiadaj¹cym mu pe³nym drzewie
binarnym, co pokazuje rysunek \ref{ref:bintree}. Chcemy analizowaæ pary kompresji, poniewa¿ ka¿da wczeœniejsza kompresja wp³ywa na nastêpn¹ i niejako przygotowuje dla niej dane.
...
Statystyki krawêdzi uaktualniane s¹ w ten sposób podczas trwania ka¿dej kompresji o ile wskaŸnik na statystyki,
ustawiony jest w drzewie. W momencie ukoñczenia kompresji statystyki powinny ju¿ byæ aktualne.
...
Wymieniaj¹c krawêdŸ algorytm w zasadzie wymienia poddrzewo w grafie, zaczynaj¹c od pocz¹tku lub koñca krawêdzi (oszczêdzaj¹c pocz¹tek). Na rysunku \ref{} widzimy oba przypadki. Na przyk³ad w pierwszej \emph{mutacji} tylko
jedna odnoga \emph{DICT} zosta³a zmieniona, podczas gdy w przejœciu drugim zosta³a wymieniona ca³a krawêdŸ \emph{AFL-NONE}.
...
dokonywane tylko jeœli wed³ug statystyk nast¹pi poprawa kompresji. W ten sposób nawet jeœli charakterystyka
...
inne optymalne drzewo. Próbujemy równie¿ je poprawiaæ i jeœli siê uda³o, to ponownie zapisujemy je jako
...
tak¿e lepiej dostosowywaæ siê do zmian danych. Taka równoleg³oœæ jest mo¿liwe, poniewa¿ statystyki
...
\subsubsection{Pomiar parametrów dzia³ania komputera MAC}
\subsubsection{Dane z High Frequency Trading NYSE}
