...
\item Dla ka¿dego w¹tku $t$ CUDA ozn. \emph{$t=thread(b_{idx},t_{idx})$}, co oznacza w¹tek o indeksie $t_{idx}$ z bloku $b_{idx}$, wyznaczany jest pocz¹tkowy indeks danych wejœciowych i wyjœciowych dla tego w¹tku. Aby wyjaœniæ jak mo¿na je wyznaczyæ wprowadŸmy nastêpuj¹ce oznaczenia:  

\begin{itemize}
	\item $\omega$ -- iloœæ w¹tków w grupie (w naszym przypadku jest to licznoœæ w¹tków w warpie, czyli 32) -- (warp size)
	\item $\kappa$ -- iloœæ bitów w s³owie (32 bity)
	\item $B_{size}$ -- wielkoœæ bloków w¹tków -- (CUDA block size)
	\item $B_{count}$ -- iloœæ bloków potrzebna do zakodowania wszystkich elementów wektora $W$ -- (CUDA block count)
	\item $W_{lane}(t) = t_{idx} (mod\ \omega)$ -- indeks w¹tku t w grupie -- (warp lane)
	\item $\Psi(t) = t_{idx} - W_{lane}(t)$ -- index pierwszego w¹tku w grupie
\end{itemize}
Wtedy indeks bloku danych dla danego w¹tku mo¿na obliczyæ za pomoc¹ wzoru $ d_{b}(t) = b_{idx} * B_{size} + \Psi(t) $ co obrazuje rysunek \ref{ref:afl1}. Wielkoœæ bloków w¹tków ustalmy na $ B_{size} = 32 * 8 $ co sprawi, ¿e ka¿dy blok bêdzie siê sk³ada³ z 8 warpów.



\begin{figure}[h!]
	\centering
	\hspace*{-0.5cm}\includegraphics[scale=0.55]{img/AFL}
	\caption{AFL - przejœcie z u³o¿enia w¹tków w bloku na bloki danych}
	\label{ref:afl1}
\end{figure}

Jak mo¿na zauwa¿yæ, ka¿demu warpowi w bloku odpowiada tyle samo bloków danych, przy czym bloki danych s¹ wielkoœci $\kappa$ elementów i ich numery s¹ potrzebne do obliczenia pocz¹tkowego indeksu wejœciowego dla w¹tku, który definiujemy jako $ \mu_{0}(t) = d_{b}(t)*\kappa + W_{lane}(t) $, gdzie kolejne indeksy wyliczane s¹ jako przesuniêcia o $\omega$, czyli $ \mu_{k}(t) = \mu_{0}(t) + k * \omega $, gdzie $ k \in [1,\kappa-1] $. Tworzy to podzia³ danych wejœciowych, który obrazuje \ref{ref:afl3}. Dziêki takiemu u³o¿eniu kolejne w¹tki z tego samego warp, wykonuj¹ odczyt kolejnych rekordów z tablicy Ÿród³owej, poniewa¿ w¹tek 0 odczytuje rekord 0, w¹tek 1 rekord 1, a¿ wreszcie w¹tek 31 rekord 31. Dziêki temu odczyt mo¿e byæ po³¹czony (Global Memory Coalescing\cite{UENG}).
	
\begin{figure}[h!]
	\centering
	\hspace*{-0.5cm}\includegraphics[scale=0.55]{img/AFL3}
	\caption{AFL - Indeksy danych wejœciowych}
	\label{ref:afl3}
\end{figure}
...
\item Po skompresowaniu, dane powinny mieæ dok³adnie $\sigma * len(W)$ bitów d³ugoœci, nie licz¹c metadanych które mo¿na zapisaæ w dwóch bajtach. 
...
\bibitem[43]{UENG} Ueng, Sain-Zee, et al. "CUDA-lite: Reducing GPU programming complexity." Languages and Compilers for Parallel Computing. Springer Berlin Heidelberg, 2008. 1-15.
