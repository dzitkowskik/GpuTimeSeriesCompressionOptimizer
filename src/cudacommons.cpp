...
bool CudaCommons::CudaCheckDeviceForRequirements(int n)
{
	...
	if(prop.major < _config->GetValue<int>(std::string("MIN_CUDA_MAJOR_VER"))) return false;
	if(prop.minor < _config->GetValue<int>(std::string("MIN_CUDA_MINOR_VER"))) return false;
