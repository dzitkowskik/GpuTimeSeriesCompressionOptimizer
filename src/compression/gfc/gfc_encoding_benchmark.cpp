...
BENCHMARK_DEFINE_F(GfcEncodingBenchmark, BM_Delta_Random_Float_Decode)(benchmark::State& state)
{
    int n = state.range_x();
    auto data = MoveSharedCudaPtr<double, char>(CudaArrayGenerator().GenerateRandomDoubleDeviceArray(n));
    GfcEncoding encoding;
...
BENCHMARK_REGISTER_F(GfcEncodingBenchmark, BM_Delta_Random_Float_Decode)->Arg(1<<22)->Arg(1<<23)->Arg(1<<24);
